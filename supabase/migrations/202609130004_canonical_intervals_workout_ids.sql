-- Make the server worker and Flutter client address the same Intervals.icu
-- calendar event. Bumping the delivery contract changes the content hash and
-- safely requeues every future workout so the worker can remove legacy UUID
-- events and upsert the canonical cycleready-YYYY-MM-DD event.
...
-- Fire the trigger once for every future prescription. The v2 contract field
-- makes this a new desired version while all outbox idempotency guarantees stay
-- in effect.
