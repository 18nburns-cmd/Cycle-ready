-- Contract v3 requeues future workouts so the delivery worker can reconcile
-- API-key, replaced-plan UUID and OAuth copies by provider event ID before it
-- keeps/upserts the authoritative OAuth event for each date.
